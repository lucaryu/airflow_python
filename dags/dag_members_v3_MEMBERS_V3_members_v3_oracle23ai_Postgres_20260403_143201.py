...
SOURCE_SQL = """
    SELECT MSNO
     ...
     , ETL_CRY_DTM
  FROM KKBOX.MEMBERS_V3
"""

# 2. 적재 테이블 이름 (스키마 제외, 테이블명만)
TARGET_TABLE = "members_v3"
...
params = {
    ...
    "target_owner": Param(TARGET_OWNER, type=["string", "null"], description="Postgres 스키마 (owner). 비우면 public 사용"),
}

with DAG(
    dag_id='dag_members_v3_MEMBERS_V3_members_v3_oracle23ai_Postgres_20260403_143201',
    ...
    load_task = S3ParquetToPostgresOperator(
        ...
        target_table='{% if params.target_owner %}{{ params.target_owner }}.{{ params.target_table }}{% else %}{{ params.target_table }}{% endif %}',
        from_date='{{ params.from_date }}',
        to_date='{{ params.to_date }}',
